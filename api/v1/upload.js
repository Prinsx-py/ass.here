...
export default async function handler(req, res) {
  ...
  const {
    ...
    duration,
    has_karaoke_fx: rawHasKaraokeFx
  } = req.body || {};
  ...
  const { data: uploadData, error: uploadError } = await supabase.storage.from(SUPABASE_BUCKET).upload(uniqueFileName, buffer, {
    cacheControl: '3600',
  ...
  const hasKaraokeFx = rawHasKaraokeFx === undefined
    ? undefined
    : normalizeBoolean(rawHasKaraokeFx);
  ...
  if (typeof hasKaraokeFx !== 'undefined') {
    insertRow.has_karaoke_fx = hasKaraokeFx;
  } else if (normalizedSourceType.toLowerCase() === 'synced') {
    insertRow.has_karaoke_fx = true;
  }

  const { data: row, error: dbError } = await supabase.from('ass_tracks').insert([insertRow]).select();
